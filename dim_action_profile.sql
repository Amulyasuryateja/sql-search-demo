...
{{
config(
materialized='incremental', incremental_strategy='delete+insert', unique_key="sk_dim_action_profile", tags=["SC_DIM"]
)

}}

/* CTE to fetch data from source object PPFTCONDE HIST for P4 instance */
with source as (
select
source_system,
mandt,
...
from {{ source("sap_erp","ppftconde_hist") }}
...
/* CTE to create a model from PPFTCONDE_HIST as per ADS document +/
final as (
select
ifnull(
{{
